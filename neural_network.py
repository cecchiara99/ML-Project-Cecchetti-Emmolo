...
class NeuralNetwork:
    def __init__(self, input_size, output_size, activation_hidden, activation_output, hidden_size=2, learning_rate=0.1, epochs=1000, batch_size=32, momentum=0.9, lambda_reg=0.01):
        ...
        self.activation_hidden = relu if activation_hidden == "relu" else sigmoid
        self.activation_output = tanh if activation_output == "leaky_relu" else tanh
        self.activation_hidden_derivative = relu_derivative if activation_hidden == "relu" else sigmoid_derivative
        self.activation_output_derivative = tanh_derivative if activation_output == "leaky_relu" else tanh_derivative


    def initialize_parameters(self):
        np.random.seed(42)
        weights_input_hidden = np.random.uniform(low=-0.1, high=0.1, size=(self.input_size, self.hidden_size))
        biases_hidden = np.ones((1, self.hidden_size))
        weights_hidden_output = np.random.uniform(low=-0.1, high=0.1, size=(self.hidden_size, self.output_size))
